...
def selectFile():
        ...
        if (fileContent != ''):
                ...
                xfile = openpyxl.load_workbook(fileContent)
                ...
                xfile.save('รายงานประจำวัน.xlsx')
...
window = Tk()
window.title('โปรแกรมทำรายงานประจำวันอัตโนมัติ')
...
# Create Buttons
button1 = Button( window, text = "เลือกไฟล์",command = lambda:[selectFile(),start()])
button2 = Button( window, text = "ดาวน์โหลดไฟล์ผลลัพธ์",command=save_file)
...
# button1 = Button(window,text="เลือกไฟล์",command= lambda:[selectFile(),start()]).pack()
# button2 = Button(window,text="ดาวน์โหลดไฟล์ผลลัพธ์",command=save_file).pack()
